...
- **번역 토글(핵심 인터랙션, 상세 화면)**: 단일 boolean 상태 `translated`(기본 `false`). 토글 시 아래가 **동시에** 스왑됨 — 제목, 저자, 초록 본문, 언어 플래그 칩(`EN 원문`↔`한국어 번역본`), 초록 라벨(`ABSTRACT`↔`초록 · 번역본`), 버튼 라벨(`번역본 보기`↔`원문 보기`). 즉시 전환(애니메이션 없음). 실제 앱에서는 **원문은 논문 소스에서, 번역은 번역 API로 온디맨드 요청**하고 로딩/실패 상태를 처리할 것.
