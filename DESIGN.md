...
### 6. 상세 리뷰 — 에이전트 리뷰 뷰어 (2개 상태)
- **Purpose**: 나노클로 paper-review 에이전트가 작성한 장문(40~81KB) **한국어 마크다운 리뷰(review.md)** 를 모바일에서 읽는 화면. 콘텐츠 계약은 동봉된 **`REVIEW-FORMAT.md`** 참고 — 구현 전 필독. 구형 템플릿(16편)도 있으므로 특정 섹션 제목에 의존하지 말고 일반 마크다운(H1~H3·표·코드블록·이미지·블록인용)이 전부 대응돼야 함.
- **상태 A — 문서 상단**: 뒤로가기("← 라이브러리") + "arXiv ↗" 링크 → 트랙/슬러그 칩(`VLN` 액센트 칩 + `AwareVLN_CVPR2026` mono 칩) → **단일 H1**(Newsreader 25 — 사이트 헤더 제목과 중복 노출 금지, 알려진 문제 #4 해결) + 영문 부제(mono 11 uppercase) → **출판 정보 카드**(label/value 행: 발표처 / arXiv(링크 색) / 코드 — ✅·❌·예정 마커 그대로 렌더 / 리뷰 일자 / 분량·예상 읽기 시간) → **목차 카드**(11개 섹션 번호+제목, 현재 섹션 하이라이트 + 진행률 %) → 하단 고정 CTA "이어서 읽기 · 34%".
- **상태 B — 본문 리딩**: 상단 고정 리딩 헤더(‹ 뒤로 + 현재 H2 섹션명(mono, ellipsis) + 진행률 % + 하단 3px 진행 바) → 본문 요소 트리트먼트:
  - 본문: 15px / lh 1.75 / `#3F3A31`, **볼드** 다수. H2/H3는 Newsreader.
  - **수식**: 인라인 수식은 serif italic(`x_t`, `h_t`), 디스플레이 수식은 white 카드(radius 12) 중앙 정렬 + 수식 번호 "(3.1)"(mono). 실제 구현은 **KaTeX 렌더링**(알려진 문제 #1 해결) — `$$…$$`와 유사표기 혼재 대응.
  - **ASCII 아키텍처 다이어그램**: 다크 코드블록(bg `#26221B`, text `#E8E1D2`, mono 10.5 / lh 1.55), `white-space:pre` + 컨테이너 `overflow-x:auto` — **줄바꿈 절대 금지**.
  - **그림**: `figures/fig_XX_XX.jpeg` 상대경로, 풀폭 + 캡션(12.5, `#948D7C`). 가로형 그림 많음.
  - **표**: 5~8열 광폭 → white 카드 컨테이너 `overflow-x:auto` + `min-width` 유지, th는 mono 라벨, 셀 `white-space:nowrap`, **Ours 행 하이라이트**(bg `#F9EFE6`, text `#A34E2E` 600).
  - 마커 이모지(✅ ❌ ⏳ 🌐)는 콘텐츠로 그대로 렌더.
  - 하단 이전/다음 섹션 네비게이션(`← 2. 문제 정의` / `4. 시스템 아키텍처 →`).
- **Behavior**: 목차 탭 → 해당 섹션으로 스크롤. 읽기 위치(진행률) 저장·복원. 라이브러리/피드에서 진입(피드 카드 데이터는 search 파일에서: 한줄 요약 → 초록, 핵심 기여 → "왜 추천했나요?", 카테고리 체크박스 → 태그). 리뷰 본문은 항상 한국어(UI `lang`과 무관). 모바일 WKWebView 폰트 스케일 문제(#2) → 본 시안의 px 값 준수.
...
- `REVIEW-FORMAT.md` — 나노클로 paper-review 에이전트의 리뷰 마크다운 양식 명세. **상세 리뷰 화면(화면 6)의 콘텐츠 계약 — 구현 전 필독.**
